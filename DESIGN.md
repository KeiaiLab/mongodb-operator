...
# DESIGN — mongodb-operator (Open Source)
...
mongodb-operator 는 **Kubernetes 의 declarative API 로 MongoDB 의 Day-0 / Day-1 / Day-2 lifecycle 을 외부 종속성 없이 운영** 한다.

**대상 사용자**:
- *Day-0*: 신규 MongoDB cluster 를 Kubernetes manifest 1개로 배포하려는 platform engineer
...
**Non-goals**:
- Atlas / DocumentDB 같은 managed service 대체 — 본 operator 는 *self-hosted MongoDB on Kubernetes*.
- mongodb-community-operator 또는 percona-server-mongodb-operator 의 fork — 본 operator 는 *독립 구현*, 별 design 정신.
- MongoDB Inc. license 종속 기능 (Enterprise Auditing, Atlas integration) — *strictly OSS* (`SSPLv1` 호환 *우회* 위해 *외부 시스템 추상화* 만 제공).

## §2 핵심 설계 원칙 (5)

### §2.1 Declarative Boundary

*K8s API 가 single source of truth*. CR 의 `spec` 이 desired state, operator 가 *reconcile loop* 으로 actual state 를 spec 에 수렴. operator 외부 mutation (mongosh, manual scale, etc.) 은 *immediately reconciled or rejected* — webhook 으로 차단.

### §2.2 Minimal Surface

K8s 표준 자원 (StatefulSet + PVC + Service + Secret) 으로 표현 가능한 것은 *그대로 사용*. operator 가 자체 type 을 만들지 않음. CRD 는 *MongoDB-specific 의 invariant* (replicaSet 멤버 수 + sharded topology + backup PITR) 만 도입.
...
LDAP / OIDC / Vault Transit / cross-cluster federation 같은 *external system integration* 은 *abstract interface* + *driver impl* 분리 (cycle 17). 신규 system 추가 시 *driver 만 작성*. Core reconciler 무영향.

## §3 CRD surface

[ARCHITECTURE.md §CRD surface](ARCHITECTURE.md) 참조. 3 owned CRDs (`MongoDB`, `MongoDBSharded`, `MongoDBBackup`) + 보조 CRDs.

각 CRD 의 `spec` 는 *user intent*, `status` 는 *operator observation* — 명확 분리.
...
- **OLM v0** — *OpenShift / OperatorHub.io community-operators* 호환. legacy, 단 community-operators 의 *기존 등록 버전 0.3.0 → 1.5.0 upgrade* 위해 *지속 release* (ADR-0027 자동화).
...
- v1.6.x (planned) = narrow OLM v1 installer RBAC + community-operators 0.3.0 → 1.5.0 upstream sync + mailstory FerretDB cutover plan (별 plan).
...
- **Patch** (`vX.Y.Z+1`): 1-2 weeks, security 또는 critical bug
- **Minor** (`vX.Y+1.0`): 4-8 weeks, new feature
...
본 design 의 *모든 비역행 결정* 은 ADR 로 보존. ADR-0001 부터 ADR-N (현재 29). `docs/kb/adr/INDEX.md` 가 SSOT.
