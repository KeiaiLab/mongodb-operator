...
1. **Openness (개방성)**: 모든 의사결정은 공개 채널 (GitHub issue / PR / RFC) 에서 이뤄집니다.
2. **Lazy Consensus (최소 합의)**: 일상적 변경은 반대 없으면 진행됩니다.
3. **Explicit Consensus (명시적 합의)**: 아키텍처 변경, CRD 변경, 보안 모델 변경, 라이선스 변경은 RFC / ADR 후 Maintainer **2/3 supermajority** 승인. 일반 RFC (단일 컴포넌트 / 도구 채택 / 정책 보강) 는 **simple majority (>50%)**. GOVERNANCE 자체 변경 (§ "본 문서 변경") 은 항상 2/3 supermajority.
4. **Shared Responsibility (공동 책임)**: Maintainer 는 코드 품질, 사용자 안전, 커뮤니티 건강에 대해 공동 책임을 집니다.

## Decision Classification (의사결정 분류)

### Routine Changes (일상 변경, Lazy Consensus)
- 버그 픽스, 문서 개선, 테스트 추가, 의존성 minor / patch 업그레이드, 리팩터링 (공개 API 무변경)
- 절차: PR → 1 명 이상 Maintainer LGTM → 머지
- 시한: 별도 코멘트 윈도우 없음 (로컬 게이트 통과 시 즉시 머지 가능 — RFC-0002 에 따라 GitHub Actions 미사용, pre-commit / pre-push hook + Makefile 로 검증)

### Medium Changes (중간 변경, Explicit Consensus)
- 새 CRD 필드 추가, 새 reconciler, 의존성 major 업그레이드, 공개 API 변경
- 절차: 이슈로 제안 → 7 일 코멘트 윈도우 → Maintainer 다수 LGTM → 머지
- 거부 1 건이 있을 시 Maintainer 회의에서 토론

### Architecture Changes (아키텍처 변경, RFC / ADR 필수)
- 새 컴포넌트 도입, 보안 모델 변경, 라이선스 변경, 호환성 깨는 변경
- 절차:
  1. `docs/kb/adr/NNNN-title.md` 에 ADR 또는 RFC 제출
  2. 14 일 코멘트 윈도우
  3. Maintainer 2/3 이상 찬성
  4. ADR / RFC Status: `Draft → Accepted` 후 구현 PR 진입

### Deployment Model Changes (배포 모델 변경, ADR + 사용자 명시 cluster apply, 2026-05-15 추가)
...
## Release Decisions (릴리스 결정)

릴리스 분기 / 버전 bump 는 Maintainer 1 인이 Lazy Consensus 로 진행 가능. 단 LTS 라인 신설 / EOL 선언 / Sharded GA 졸업 등 *주요 마일스톤* 은 Explicit Consensus 필수.
