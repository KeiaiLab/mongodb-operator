...
[Bitnami `mongodb-sharded` 9.4.12 同等性分析](docs/comparison/bitnami-mongodb-sharded.md) 9 件のギャップ。Helm chart ユーザーが本 Operator へ *漏れなく 1:1 マイグレーション* できる必要があります。
...
- [Bitnami `mongodb-sharded` 同等性分析](docs/comparison/bitnami-mongodb-sharded.md)
...
- [x] **OLM v1 (operator-controller v1.8) 採用** (ADR-0029, 2026-05-15) — *モダン標準* (next-generation、2026-02 GA)。`deploy/olm-v1/` (ClusterCatalog + ClusterExtension + installer SA + cluster-admin binding) + `INSTALL.md` 3-path matrix。ライブ検証: KeiaiLab Cluster — `ClusterExtension mongodb-operator` Installed=True / Succeeded、operator pod Running v1.5.0、helm chart + mailstory-ferretdb 無影響。v0 (`olm` ns + 7 CRD) cleanup 完了。
