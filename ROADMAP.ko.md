...
각 sub-task 우측 *Verify* 는 검증 명령 또는 e2e 파일을 인용합니다.

## 현재 상태 (Current Status — v1.0.0)

### 핵심 기능 — 구현 완료 (Core Features — Implemented)
...
## MongoDB Enterprise 비교 (Enterprise Comparison)
...
## Phase 1 — 프로덕션 강화 (Production Hardening)

**목표 (Goal)**: 프로덕션 환경의 안정성·운영성 개선.

### 1.1 Point-in-Time Recovery (PITR) 완전 구현 (Full Implementation)
...
### 1.2 Grafana 대시보드 템플릿 (Dashboard Templates)
...
### 1.3 자동 버전 업그레이드 (롤백 포함) (Automated Version Upgrade with Rollback)
...
### 1.4 확장 모니터링 메트릭 (Extended Monitoring Metrics)
...
## Phase 2 — 엔터프라이즈 인증 + 고급 운영 (Enterprise Auth + Advanced Ops)

**목표 (Goal)**: 엔터프라이즈 보안 표면 + 다중 리전.

### 2.1 LDAP 인증 지원 (LDAP Authentication Support)
...
### 2.2 OIDC / OAuth2 인증 (OIDC / OAuth2 Authentication)
...
### 2.3 다중 리전 지원 (`MongoDBFederation`) (Multi-Region Support)
...
### 2.4 저장 데이터 암호화 (KMS) (Encryption at Rest)
...
## Phase 3 — 고급 엔터프라이즈 기능 (Advanced Enterprise Features)

**목표 (Goal)**: 엔터프라이즈급 운영 역량.

### 3.1 고급 백업 기능 (Advanced Backup Capabilities)
#### 3.1.1 쿼리 가능한 백업 (Queryable Backup)
...
#### 3.1.2 대역폭 제한 (Bandwidth Throttling)
...
#### 3.1.3 자동 백업 검증 (Automated Backup Verification)
...
### 3.2 성능 분석 도구 (`MongoDBInsights`) (Performance Analysis)
...
### 3.3 멀티 클러스터 관리 (`MongoDBClusterGroup`) (Multi-Cluster Management)
...
### 3.4 고급 감사 로깅 (Advanced Audit Logging)
...
## Phase 4 — Bitnami `mongodb-sharded` Helm chart 동등성 (Helm Chart Parity)

[Bitnami `mongodb-sharded` 9.4.12 동등성 분석](docs/comparison/bitnami-mongodb-sharded.md) 9건 갭. Helm chart 사용자가 본 Operator 로 *누락 없이 1:1 마이그레이션* 가능해야 합니다.

### 4.1 NetworkPolicy 자동 생성 (P0) (Auto-Generated NetworkPolicy)
...
### 4.2 Sharded Arbiter / Hidden member (P0) (Sharded Arbiter / Hidden Member)
...
### 4.3 워크로드 사이드카 · extraVolumes · extraEnvVars 주입 (P1) (Workload Sidecar / extraVolumes / extraEnvVars Injection)
...
### 4.4 PVC retention policy 노출 (P1) (PVC Retention Policy Exposure)
...
### 4.5 volumePermissions init container (P1) (volumePermissions Init Container)
...
### 4.6 Init scripts ConfigMap (P2) (Init Scripts ConfigMap)
...
### 4.7 Service 옵션 확장 (P2) (Service Option Extension)
...
### 4.8 Diagnostic mode + Resource presets (P2) (Diagnostic Mode + Resource Presets)
...
### 4.9 Scale-in / Member removal (P2) (Scale-in / Member Removal)
...
### 높은 가치, 낮은 난이도 (즉시 실행) (High Value, Low Difficulty — Immediate)
- ✅ Grafana 대시보드 템플릿
- ✅ 확장 모니터링 메트릭
- ✅ 4.4 PVC retention (필드 존재, 매핑만)

### 높은 가치, 높은 난이도 (전략적 투자) (High Value, High Difficulty — Strategic Investment)
- 🎯 PITR 완전 구현
- 🎯 LDAP / OIDC 인증
- 🎯 다중 리전 (`MongoDBFederation`)
- 🎯 성능 분석 (`MongoDBInsights`)

### 낮은 가치, 낮은 난이도 (빠른 성과) (Low Value, Low Difficulty — Quick Wins)
- 📝 4.7 Service 옵션 확장
- 📝 4.8 Diagnostic mode + presets

### 낮은 가치, 높은 난이도 (회피) (Low Value, High Difficulty — Avoid)
...
다음은 MongoDB Enterprise 바이너리가 필요하므로 *구현하지 않습니다*:
...
Enterprise 기능이 필요한 경우 MongoDB Enterprise Operator 사용을 권장합니다.
...
본 ROADMAP 은 살아있는 문서이며, 커뮤니티 피드백과 코드 사실에 따라 갱신됩니다.

---

## Phase 5 — Post-v1.5.0 (candidate baseline, brainstorm pending)

> Phase 1-4 100% (93/93) 마감 후 *후속 가치 영역*. 본 section 은 `~/.claude/plans/2026-05-14-4-operators-100pct/P-E.md` 의 후보 6 카테고리 (observability v2 / DR / sharded v2 / security v2 / commons import / community) 를 *기준 baseline* 으로 등재합니다. 사용자 brainstorm session 합의 후 확정 / 교체 / 통합합니다.

### 5.1 Production observability v2 (Production Observability v2)
...
### 5.2 Disaster recovery 고도화 (Disaster Recovery Enhancement)
...
### 5.3 Sharded topology v2 (Sharded Topology v2)
...
### 5.4 Security hardening v2 (Security Hardening v2)
...
### Brainstorm gate (Brainstorm Gate)
