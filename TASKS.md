...
| I26 | mongodb I16 결정 — MonitoringSpec orphan 의 a/b/c 옵션 ADR | 발견 | 0% | I16 | F15, valkey 패턴 | 2026-05-07 it cluster-ops audit. valkey 가 controller-level 동적 ServiceMonitor 생성 (commons.monitoring 위임 + fail-soft applyServiceMonitor) 기준. mongodb 의 I16 결정 (a) spec 삭제 / (b) valkey 패턴 차용 구현 (M4) / (c) deprecation marker → ADR-0018 작성 후 결정. cross-cut 통일 (postgres 도 동일 추가). |
