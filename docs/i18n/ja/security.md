...
本プロジェクトは MIT License の下でライセンスされています。同ライセンスに従い、本ソフトウェアは「現状のまま (AS IS)」提供されており、明示または黙示を問わず、MERCHANTABILITY、FITNESS FOR A PARTICULAR PURPOSE、NONINFRINGEMENT を含むがそれに限定されない、いかなる種類の保証も付されません。
