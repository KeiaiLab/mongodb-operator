...
- *OpenShift 4.x / OperatorHub.io community-operators 사용자*: 지원이 종료되었습니다. helm 또는 OLM v1 으로 마이그레이션이 필요합니다.
...
- private registry pull credentials (현재 release 의 catalog/bundle image 가 `internal` GHCR — public 화 또는 imagePullSecret 필요)
...
---
# *Note*: production 에서는 narrow ClusterRole 사용을 권장합니다 (bundle-derived).
# 본 예제는 *cluster-admin* 단순화 — installer 자체의 권한 범위가 광범위합니다.
...
# 또는 channel-based — catalog 가 stable 채널의 새 version 을 publish 하면 자동 upgrade
kubectl patch clusterextension mongodb-operator --type=merge \
...
spec:
  ...
  syncPolicy:
    ...
    syncOptions:
      ...
      - CreateNamespace=false  # namespace.yaml 에 명시되어 있음
```
...
`charts/mongodb-operator/values.yaml` 를 참조하세요. 주요 옵션:
...
spec:
  ...
  replicaSetName: rs0
  storage:
    storageClassName: ceph-block  # 또는 환경별 storage class
    size: 50Gi
  resources:
