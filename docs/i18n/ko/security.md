...
1. **GitHub Security Advisory** (권장):
   `https://github.com/keiailab/mongodb-operator/security/advisories/new`
2. **이메일**: `security@keiailab.com` (PGP 옵션):
   - PGP fingerprint: `89A4 0947 6828 CB99 2338  C378 651E 51AF 520B CB78`
   - public key: gh-pages 의 `artifacthub-repo.yml` 또는 https://keiailab.github.io/mongodb-operator/artifacthub-repo.yml
...
COSIGN_EXPERIMENTAL=1 cosign verify \
  --certificate-identity-regexp '^https://github\.com/keiailab/mongodb-operator/\.github/workflows/release\.yml@' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com \
  ghcr.io/keiailab/mongodb-operator:<version>
```
...
slsa-verifier verify-image \
  --source-uri github.com/keiailab/mongodb-operator \
...
Download `mongodb-operator-<version>.tgz`, `.tgz.sig`, and `.tgz.pem`
from the GitHub Release page, then:

```bash
cosign verify-blob \
  --certificate   mongodb-operator-<version>.tgz.pem \
  --signature     mongodb-operator-<version>.tgz.sig \
  --certificate-identity-regexp '^https://github\.com/keiailab/mongodb-operator/\.github/workflows/release\.yml@' \
  --certificate-oidc-issuer https://token.actions.githubusercontent.com \
  mongodb-operator-<version>.tgz
```
