...
3. Helm chart auto-publish + release supply-chain
...
- valkey-operator (ADR-0048 Accepted) 는 GHA 유지 노선을 별도 선택 — operator 별 trade-off 다르게 처리.
- postgres-operator + mongodb-operator 는 SPOF risk 최소화 우선 — 2026-04-28 incident
  (org billing → 전 repo 24h+ merge freeze) 의 재발 회피가 external trust signal 손실보다 우선.

이로써 ADR-0031 의 *유지* 노선은 폐기되고, 본 ADR 이 strict 적용을 SSOT 화한다.

## Decision

`.github/workflows/` 전면 제거 (12 파일) + 모든 게이트를 **로컬 4계층** 으로 일원화:

| Layer | 메커니즘 | 본 repo 의 구체 도구 |
|---|---|---|
| L1 pre-commit hook | `.lefthook.yml pre-commit` | gofmt, govet, golangci-lint (`--new-from-rev`), helm-lint (chart 변경 시) |
| L2 pre-push hook | `.lefthook.yml pre-push` | unit-test, full-lint, helm-lint, helm-template, govulncheck, gitleaks, platforms-amd64-guard, go-mod-tidy, **kube-linter** (NEW), **go-licenses** (NEW), **markdown-link-check** (NEW) |
...
- forgewise-commons 등 internal-infra repo 와 정합 (이미 RFC-0002 strict 적용).
- postgres-operator (ADR-0018) 와 정합 — 두 RDBMS-flavor operator 가 동일 노선.
...
- ADR-0031 (Proposed) 의 분류 체계 자체는 유효 — 그러나 본 ADR 은 *전부 제거* 노선이므로 ADR-0031 폐기와 함께 무효화.

**Neutral**:

- valkey-operator (ADR-0048 Accepted) 와 정책 분기 — operator 별 trade-off 차이 인정. 향후 SPOF risk 평가 결과에 따라 두 노선 재정렬 가능.

## Alternatives Considered

1. **ADR-0031 (GHA 12개 유지 + dual operation)** — Proposed → Superseded by this ADR.
...
3. **valkey-operator 와 동일 노선 (전부 유지)** — Rejected.
   postgres + mongodb 는 internal-use 비중이 더 크고 external contributor PR cadence 가 valkey 보다 낮음.
   SPOF risk 회피 우선 결정.

## References

- **RFC-0002** (2026-04-29) — Global GHA 영구 금지.
- **ADR-0031** (Superseded) — Public OSS 유지 노선 (폐기).
- **ADR-0012** — Renovate 단일 의존성 봇 채택 (GHA 의존 없이 동작).
- **ADR-0027** — community-operators sync 자동화 (수동 수행).
- **Cross-operator**:
  - `postgres-operator/docs/kb/adr/0018-gha-to-local-4-layer.md` (Accepted, 정합 — S7 cycle 동시 진행).
  - `valkey-operator/docs/kb/adr/0048-gha-retention-for-public-oss.md` (Accepted, 유지 노선 — 분기).
- **Incident KB**: I-2026-04-28 (GHA billing outage; RFC-0002 trigger).

## Implementation

- **Phase 1 (PR #194, 머지)**: `.github/workflows/` 12 파일 `git rm -r`.
- **Phase 2 (PR #195, 머지)**: `scripts/helm-publish.sh` + `scripts/release.sh` 신규.
- **Phase 3 (PR #196, 머지)**: lefthook pre-push 3종 보강 + Makefile `kube-lint`/`go-licenses`/`md-link-check` target 추가.
- **Phase 4 (본 ADR PR)**: ADR-0032 Accepted, ADR-0031 Superseded.

Status: Proposed → Accepted (본 ADR PR 머지 시점).

## 변경 이력

- **2026-05-21 Accepted**: S7 cycle 4 phase 완료. ADR-0031 supersede. RFC-0002 strict
  적용 완료 — 로컬 4계층 단일 운영 + 12 workflow 제거 + 3종 보강 (kube-linter,
