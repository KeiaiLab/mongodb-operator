# ADR-0035: RFC-0002 GitHub Actions Block — lefthook pre-commit hook 자동 강제
...
| Author | keiailab |
| Supersedes | (none) |
| Related | RFC-0002 (GitHub Actions Permanent Ban), ADR-0033 (GHA 유지 + operator family v2.0 dual-track), commons ADR-0012 (gha-block hook 패턴 SSOT), postgres ADR-0021 (sister) |
...
ADR-0033 (Accepted, 2026-05-21) 결정: mongodb-operator 는 v2.0 = GHA *유지* (`.github/workflows/` 12 파일) + 로컬 4계층 보강 + dual-track 운영.
...
`.lefthook.yml` 의 `pre-commit.commands.gha-block` 신설 (commons ADR-0012 패턴 + postgres ADR-0021 sister):
...
gha-block:
  run: |
    ...
    if [ -n "$added" ]; then
      echo "❌ RFC-0002 §2 위반: .github/workflows/ 신규 파일 추가 금지 (modify 는 허용 — ADR-0033 v2.0 dual-track)"
      echo "   파일: $added"
...
- ✅ dependabot/renovate 의 기존 12 workflow 갱신 정상 (modified 만 — 차단 안 함, ADR-0033 정합)
- ✅ commons 의 PLAN_BYPASS 패턴 일관 적용 — `.lefthook.yml` 안 기존 hook 들의 우회 메커니즘과 동일
- ✅ ADR-0033 의 dual-track 운영 (GHA 유지 + 로컬 4계층) 과 정합 — 본 hook 은 *신규* 차단만, 기존 12 workflow 와 무관
...
- commons ADR-0012 + postgres ADR-0021 패턴이 pre-commit — sister consistency
...
기각:
- ADR-0033 dual-track 정합 위반 — 12 workflow 의 정상 유지/갱신 차단됨
- dependabot/renovate PR 이 머지 불가 상태에 빠짐
- commons ADR-0012 + postgres ADR-0021 도 add-only — 일관
...
```bash
# 기존 workflow 에 주석 추가
echo "# test modify" >> .github/workflows/ci.yml
