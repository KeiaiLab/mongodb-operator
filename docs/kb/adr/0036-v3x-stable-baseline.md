# ADR-0036: v3.x-stable baseline 인정 (audit ❌ 0 충족)
...
| Author | keiailab |
| Supersedes | (none) |
| Related | commons-ADR/0013 (audit SSOT), CLAUDE.md §7 (v3.x-stable 정의), mongodb-ADR/0035 (RFC-0002 GHA block hook), mongodb-ADR/0033 (GHA retention) |

## Context

CLAUDE.md §7: "본 규약은 **상용 제품 수준**의 다중 프로젝트 일관성을 목표로 한다 — `standards/enforcement.md`의 P0+P1+P2 자동화 모두 충족 시 *v3.x-stable* 선언."

본 repo (mongodb-operator) 는 다음 두 축으로 *v3.x-stable* 진입 조건을 충족했다.
...
- P2 (거버넌스): ✅ ADR coverage (0001~0035) / RFC-0002 GHA block hook 강제 / 표준 모듈 정합
...
본 repo (`keiailab/mongodb-operator`) 를 **v3.x-stable** 로 인정한다.

- *외부 사용자 대상 운영 등급* 으로 공개 가능.
- 후속 release tag `vX.Y.Z` 권장 — 구체 버전 (현 v1.4.x → v1.5.0 minor bump 또는 v2.0.0 major) 은 별 사용자 결정 + 별 ADR 로 추적 (CHANGELOG 정합 + semver 판단).
...
- **OLM v1 entry** — ADR-0029/0030 의 OLM v1 narrow installer 정합으로 OperatorHub.io 외부 사용자 production-grade 진입.
...
- *현 v1.4 단계 인정* vs *v2.0 GA 격상* — 본 ADR 은 격상 강제 안 함. CHANGELOG 정합 + 사용자 결정.

## 후속 (v3.1+)

본 baseline 후 v3.1+ 진화 후보:
- P3 성능 게이트 (benchmark + budget) — 별 RFC
- P4 DR 게이트 (backup + restore + chaos) — 별 RFC (mongodb backup PR 의 자연스러운 확장)
