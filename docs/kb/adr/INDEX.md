...
| [ADR-0042](0042-mongod-exec-probe-timing.md) | mongod exec 프로브 시간축 확대 — liveness 10s/5s/6 → **30s/15s/4**, readiness 10s/5s → **15s/10s**, mongosh 전 프로브 `--norc`. 사유: exec 프로브가 mongosh(Node.js)를 띄우느라 노드 exec 지연 스파이크에 걸려 **건강한 mongod 를 SIGKILL** 했다(실측 exec 타임아웃 10,007건 중 99.6%가 단일 노드, 파드 1본당 재시작 148회, CPU 스로틀 1%). liveness 는 hang 감지 위해 exec 유지(TCP 는 멈춘 mongod 도 accept). 가드 = `builder_probe_timing_test.go` | Accepted | 2026-08-25 |
