...
| **C37** | MongoDBSharded CR status conditions 빈약 (RS 는 정상) | 진행 60% | operational visibility 격차 — argos-mongo (sharded) 가 ReconcileError 1건만 vs valkey 5 / postgres 4. mongodb (RS) controller 는 이미 풍부 (Ready / ReplicaSetInitialized 등) | 1차 commit `c12d20e`: Sharded Ready / Progressing 추가 (3 conditions). 2차 commit `e3d6923`: ConfigServerReady / ShardsReady / MongosReady 추가 (6 conditions, valkey/postgres 초과 도달). 잔여 (40%): TLSReady / BackupReady (조건부 spec 활성 시) + envtest round-trip 의 condition 검증 추가. | Medium |
