...
func TestExpandDataPVCs_grows_when_SC_allows(t *testing.T) {
	c := fake.NewClientBuilder().WithScheme(resizeScheme(t)).WithObjects(
		scAllow("gp3", true),
		boundPVCFor("data-mdb-rs0-0", "ns", "gp3", "10Gi"),
		boundPVCFor("data-mdb-rs0-1", "ns", "gp3", "10Gi"),
		boundPVCFor("data-mdb-rs0-2", "ns", "gp3", "10Gi"),
	).Build()

	if err := expandDataPVCs(ctxBg(), c, "ns",
		[]string{"mdb-rs0"}, resource.MustParse("20Gi")); err != nil {
		t.Fatalf("err: %v", err)
	}
	for _, n := range []string{"data-mdb-rs0-0", "data-mdb-rs0-1", "data-mdb-rs0-2"} {
...
func TestExpandDataPVCs_skips_non_matching_PVCs(t *testing.T) {
	c := fake.NewClientBuilder().WithScheme(resizeScheme(t)).WithObjects(
		scAllow("gp3", true),
		boundPVCFor("data-mdb-rs0-0", "ns", "gp3", "10Gi"),
		boundPVCFor("data-other-mdb-0", "ns", "gp3", "10Gi"),
	).Build()
	if err := expandDataPVCs(ctxBg(), c, "ns",
		[]string{"mdb-rs0"}, resource.MustParse("20Gi")); err != nil {
		t.Fatalf("err: %v", err)
	}
	if got := sizeOf(t, c, "ns", "data-mdb-rs0-0"); got != "20Gi" {
		t.Errorf("expected 20Gi, got %s", got)
	}
	if got := sizeOf(t, c, "ns", "data-other-mdb-0"); got != "10Gi" {
		t.Errorf("other mdb PVC 변경되면 안됨: got %s", got)
	}
}
...
func TestExpandDataPVCs_no_expansion_when_SC_disallows(t *testing.T) {
	c := fake.NewClientBuilder().WithScheme(resizeScheme(t)).WithObjects(
		...
		boundPVCFor("data-mdb-rs0-0", "ns", "standard", "10Gi"),
	).Build()
	_ = expandDataPVCs(ctxBg(), c, "ns",
		[]string{"mdb-rs0"}, resource.MustParse("20Gi"))
	if got := sizeOf(t, c, "ns", "data-mdb-rs0-0"); got != "10Gi" {
...
func TestExpandDataPVCs_zero_desired_noop(t *testing.T) {
	...
	_ = expandDataPVCs(ctxBg(), c, "ns",
		[]string{"mdb-rs0"}, resource.Quantity{})
	if got := sizeOf(t, c, "ns", "data-mdb-rs0-0"); got != "10Gi" {
